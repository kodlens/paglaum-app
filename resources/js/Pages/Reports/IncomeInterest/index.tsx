...
export default function ReportIncomeInterest() {
  ...
  const loadReport = () => {
    ...
    axios.get(`/reports/get-income-interest?${params}`).then(res=>{
    ...
    <>
      ...
      <div className='min-h-screen p-6'>
        <div className='mx-auto max-w-6xl'>
          ...
          <div className='font-bold text-center mb-4'>INCOME INTEREST</div>
          <table className='border w-full'>
            <thead className='bg-gray-100 font-bold text-left'>
              <tr>
                ...
                <th className='py-2 px-6'>INTEREST EXPECTED</th>
                <th className='py-2 px-6'>INTEREST COLLECTED</th>
                <th className='py-2 px-6'>INTEREST BALANCE</th>
               ...
               {Array.isArray(data) && data.length > 0 ? (
                data.map((item: Fields, ix: number) => (
                  <tr key={ix} className="border">
                    ...
                    <td className="px-6 py-2">{item?.loan_count}</td>
                    <td className="px-6 py-2">&#8369; {item?.interest_expected?.toLocaleString() }</td>
                    <td className="px-6 py-2">&#8369; {item?.interest_collected?.toLocaleString() }</td>
                    <td className="px-6 py-2">&#8369; {item?.interest_balance?.toLocaleString() }</td>
                  </tr>
                ))
              ) : (
                <tr>
