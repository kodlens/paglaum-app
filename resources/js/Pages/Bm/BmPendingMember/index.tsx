...
const BmPendingMemberIndex = ({ auth }: PageProps) => {
  ...
  const loadDataAsync = async () => {
    ...
    try {
      const res = await axios.get<PaginateResponse>(`/bm/get-pending-members?${params}`);
  ...
  const handleClickAllowDisallow = (user: any) => {
    ...
    if (user.is_loan_allowed > 0) {
      modal.confirm({
        title: 'Disallow?', content: 'Are you sure you want to disallow this member to loan?',
        onOk: () => {
      ...
      modal.confirm({
        title: 'Allow?', content: 'Are you sure you want to allow this member to loan?',
        onOk: () => {
...
export default BmPendingMemberIndex;
